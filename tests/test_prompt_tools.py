...
async def test_get_prompt_fix() -> None:
    ...
    async with stdio_client(
        ...
        async with ClientSession(read, write) as session:
            ...
            assert "get_prompt_fix" in tool_names, (
            ...
            assert "Issue: Test issue" in response_text, (
...
async def test_get_prompt_initial() -> None:
    ...
    async with stdio_client(
        ...
        async with ClientSession(read, write) as session:
            ...
            assert tools is not None, "Tools list should not be None"
            tool_names = [tool.name for tool in tools.tools]
            assert "get_prompt_initial" in tool_names, (
            ...
            # Verify the response contains expected content
            response_text = result.content[0].text
            assert "Project: Test project" in response_text, (
...
async def test_get_prompt_proceed() -> None:
    ...
    async with stdio_client(
        ...
        async with ClientSession(read, write) as session:
            ...
            assert tools is not None, "Tools list should not be None"
            tool_names = [tool.name for tool in tools.tools]
            assert "get_prompt_proceed" in tool_names, (
                "get_prompt_proceed tool should be available"
            )

            # Call the tool and verify the response
            result = await session.call_tool(
                "get_prompt_proceed", {"task": "Test task"}
            )
            assert result is not None, "get_prompt_proceed response should not be None"
            assert len(result.content) > 0, "get_prompt_proceed should return content"

            # Verify the response contains expected content
            response_text = result.content[0].text
            assert "Task: Test task" in response_text, (
...
async def test_get_prompt_proceed_version() -> None:
    ...
    async with stdio_client(
        ...
        async with ClientSession(read, write) as session:
            ...
            # Call the tool with version parameter and verify the response
            result = await session.call_tool(
                "get_prompt_proceed", {"task": "Test task", "version": "1.0.0"}
            )
            assert result is not None, "get_prompt_proceed response should not be None"
            assert len(result.content) > 0, "get_prompt_proceed should return content"
